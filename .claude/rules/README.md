...
Four active agents live in this repo (`flow-ui/.claude/agents/`); one related agent lives at the monorepo root and handles a different concern:

- **`design-builder`** (this repo) — writes prototypes to staging, applies revisions, promotes signed-off files into the design system, **and syncs the JSX visual-review kit after promote** (`sync-kit` Phase 6.5). Also opt-in push to Figma.
...
- **`story-coverage-auditor`** (this repo) — read-only **bidirectional** story-vs-build inspector; diffs PM user stories (`_pm-plan/docs/stories/`) against staging HTML in BOTH directions. Tags `[AC-MISSING] / [AC-UNRESOLVED] / [STORY-MISSING-CRITICAL] / [STORY-MISSING-BP] / [AC-DRIFT]`. Writes per-epic reports + a separate PM-revise spec to `reports/story-coverage/`. Its design-side signoff (`Design-side AC clean: YES`) is gate G12 for promotion. PM-side findings flow back to `story-pipeline` (advisory, do NOT block promote). **PM contract enforcement** — asks "does the build match the story BDD scenarios, and is the story complete relative to what's built?"

> **G11 / `design-coverage-auditor` was retired 2026-05-17** (`.claude/agents/_retired/2026-05-17_design-coverage-auditor.md`) — directionality inverted, treated JSX kit as the contract. The JSX kit is a DOWNSTREAM visual-review SPA refreshed by `design-builder sync-kit` Phase 6.5 after promote — not a contract. G12 is the sole feature-completeness gate.
...
- **`address-pr-comments`** (this repo) — resolves human reviewer comments on an open flow-ui PR end-to-end (gather → plan → fix HTML/tokens → G10 re-lint → push to the PR branch → reply). Self-sufficient; **never** promotes, pushes to Figma, or merges. Distinct from `pr-reviewer` (produces reviews) and `pr-autopilot` (mechanical pre-merge loop). **PR comment resolution** — asks "are the reviewer's asks now addressed?"
...
The default loop: `design-builder` writes → `design-linter` grades → founder reads roll-up → `design-builder revise …` → `design-linter` re-grades → clean → `story-coverage-auditor audit epic N` (G12) → design-side clean → `design-builder promote …` → `design-builder sync-kit epic N` (Phase 6.5 — JSX visual SPA mirrors the new screens) → founder opens PR → `pr-reviewer <PR#>` posts integrity check → founder merges. Nothing lands in the design system without both signoffs (G10 linter + G12 story); nothing merges into `main`/`develop` without (at minimum) a PR review pass.

When mechanical fixes alone aren't enough (judgment calls, brand feel, layout instinct), invoke `design-reviewer` to run the subjective loop in parallel. The four active agents compose; they do not overlap.
