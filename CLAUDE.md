...
| [`pr-autopilot`](./.claude/agents/pr-autopilot.md) | End-to-end PR merge agent. Waits for CI, reads `agriflow-design-review` structured findings, auto-fixes mechanical P0/P1 (max 3 iters), escalates non-mechanical to founder, merges when clean. Invoked via `/goal`. | `/goal pr-autopilot <PR#>` / `pr-autopilot cancel <PR#>` | (orchestrates `pr-reviewer` output; never pushes to main) |
| [`address-pr-comments`](./.claude/agents/address-pr-comments.md) | Resolves a flow-ui PR's human review comments end-to-end (gather → plan → fix HTML/tokens → G10 re-lint → push to the PR branch → reply). **Never** promotes, pushes to Figma, or merges. | `/address-pr-comments <PR# \| url \| comment>` | (resolves comments on an open PR; not a promote/merge gate) |
